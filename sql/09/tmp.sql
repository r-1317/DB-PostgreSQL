...
CREATE TABLE p_characters (
  ...
  hp_max INTEGER NOT NULL CHECK (0 <= hp_max),
  hp INTEGER NOT NULL CHECK (0 <= hp) CHECK (hp <= hp_max),
  blood_type VARCHAR(2) CHECK (blood_type IN ('A', 'O', 'B', 'AB')),
  height_cm DECIMAL(4, 1) NOT NULL CHECK (50.0 <= height_cm) CHECK (height_cm <= 250.0),
  created_on DATE DEFAULT CURRENT_DATE NOT NULL CHECK ('2025-12-01' <= created_on) CHECK (created_on <= CURRENT_DATE),
